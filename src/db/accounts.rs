...
pub async fn account_exists(pool: &PgPool, account_name: &str) -> bool {
    let result = sqlx::query_scalar::<_, i64>(
        "SELECT COUNT(*) FROM accounts WHERE UPPER(name) = UPPER($1)"
    )
    .bind(account_name)
    .fetch_one(pool)
    .await;
...
pub async fn load_account(pool: &PgPool, account_name: &str) -> Result<AccountData, String> {
    ...
    let chars: Vec<(String,)> = sqlx::query_as(
        "SELECT name FROM characters WHERE account_id = $1 ORDER BY slot"
    )
    .bind(id)
    .fetch_all(pool)
    .await
...
pub async fn update_password(pool: &PgPool, account_name: &str, new_password_hash: &str) -> Result<(), String> {
    sqlx::query(
...
pub async fn set_account_banned(pool: &PgPool, account_name: &str, banned: bool, reason: &str) -> Result<(), String> {
    sqlx::query(
