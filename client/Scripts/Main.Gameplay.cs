...
public partial class Main
{
	...
	private async Task ReturnToCharSelectAsync()
	{
		...
		try
		{
			...
			await ConnectAndLogin(_lastLoginAccount, _lastLoginPassword, loginAttemptId);
		}
		catch (Exception ex)
		{
	...
	private bool IsLoginAttemptCurrent(AoTcpClient tcp, int attemptId)
	{
		return attemptId == _loginAttemptId && ReferenceEquals(_tcp, tcp);
	}

	private async Task ConnectAndLogin(string account, string password, int attemptId)
	{
		AoTcpClient? tcp = _tcp;
		if (tcp == null) return;
		...
		try
		{
			...
			if (!IsLoginAttemptCurrent(tcp, attemptId))
			{
				tcp.Dispose();
				return;
			}
			...
			if (!IsLoginAttemptCurrent(tcp, attemptId)) return;
			tcp.SendPacket(ClientPackets.WriteHardwareCheck());

			await Task.Delay(50);
			if (!IsLoginAttemptCurrent(tcp, attemptId)) return;
			tcp.SendPacket(ClientPackets.WriteAccountLogin(account, password));
			...
			_ = Task.Run(async () =>
			{
				...
				if (IsLoginAttemptCurrent(tcp, attemptId)
					&& _state.CurrentScreen == Screen.Login && !_connecting
					&& _loginForm?.StatusLabel?.Text == "Enviando login...")
				{
					CallDeferred(nameof(LoginTimeout), attemptId);
				}
			});
		}
		catch (Exception ex)
	...
	private void LoginTimeout(int attemptId)
	{
		if (_tcp == null || attemptId != _loginAttemptId) return;
