...
public partial class StartupWindowSmoke : Node
{
    public override async void _Ready()
    {
        try
        {
            ...
            GD.Print($"[STARTUP-SMOKE] PASS {(headless ? "headless layout" : $"native fullscreen {screenSize}")}, login ready in {elapsed.Elapsed.TotalSeconds:F2}s (engine initialization excluded)");
            GetTree().Quit();
        }
