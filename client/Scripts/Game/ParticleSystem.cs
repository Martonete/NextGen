    ...
    private static void UpdateStream(ParticleStream stream, ParticleStreamDef def, float deltaMs)
    {
        ...
        for (int i = 0; i < stream.Particles.Length; i++)
        {
            ...
            if (def.Gravity > 0)
            {
                ...
                // Bounce when particle reaches the floor (spawn area bottom + one tile).
                // p.Y > 0 was the original broken condition — always true on first downward step.
                if (p.Y > (def.Y2 + 32f))
                {
                    // Ensure velocity is upward regardless of how BounceStrength is signed in the INI
                    p.VelY = -Math.Abs(def.BounceStrength);
                }
            }
