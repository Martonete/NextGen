...
pub(super) async fn handle_slash_trabajando(state: &mut GameState, conn_id: ConnectionId) {
    ...
    if names.is_empty() {
        ...
        state.send_console(
            conn_id,
            &format!("Trabajando ({}): {}", names.len(), names.join(", ")),
            font_index::INFO,
        );
    }
}
...
pub(super) async fn handle_slash_ocultando(state: &mut GameState, conn_id: ConnectionId) {
    ...
    if names.is_empty() {
        ...
        state.send_console(
            conn_id,
            &format!("Ocultos ({}): {}", names.len(), names.join(", ")),
            font_index::INFO,
        );
    }
}
...
pub(super) async fn handle_slash_seguir(
    state: &mut GameState,
    ...
    state.send_console(
        conn_id,
        &format!("Siguiendo a {}.", target_name),
        font_index::INFO,
    );
}
...
pub(super) async fn handle_slash_aceptconse(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    if !is_armada {
        state.send_console(
            conn_id,
            "El jugador no pertenece a la Armada Real.",
            font_index::INFO,
        );
        return;
    }
    ...
    state.send_console(
        conn_id,
        &format!("{} ahora es consejero real.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Has sido aceptado en el Consejo Real.",
        font_index::INFO,
    );
}

/// /ACEPTCONSECAOS <name> — Accept player into Chaos council. Requires DIOS+.
pub(super) async fn handle_slash_aceptconsecaos(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    if !is_caos {
        state.send_console(
            conn_id,
            "El jugador no pertenece a las Fuerzas del Caos.",
            font_index::INFO,
        );
        return;
    }
    ...
    state.send_console(
        conn_id,
        &format!("{} ahora es consejero del caos.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Has sido aceptado en el Consejo del Caos.",
        font_index::INFO,
    );
}

/// /KICKCONSE <name> — Kick player from council. Requires DIOS+.
pub(super) async fn handle_slash_kickconse(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    state.send_console(
        conn_id,
        &format!("{} ha sido removido del consejo.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Has sido removido del consejo.",
        font_index::INFO,
    );
}

/// /ESTUPIDO <name> — Mute player (prevent speech). Requires SEMIDIOS+.
pub(super) async fn handle_slash_estupido(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    state.send_console(
        conn_id,
        &format!("{} ha sido silenciado.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Has sido silenciado por un GM.",
        font_index::INFO,
    );
...
pub(super) async fn handle_slash_noestupido(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    state.send_console(
        conn_id,
        &format!("{} ya no esta silenciado.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Tu silencio ha sido removido.",
        font_index::INFO,
    );
}
...
pub(super) async fn handle_slash_trigger(state: &mut GameState, conn_id: ConnectionId, args: &str) {
    ...
    if trigger_val < 0 || trigger_val > 255 {
        state.send_console(
            conn_id,
            "Valor de trigger invalido (0-255).",
            font_index::INFO,
        );
        return;
    }
...
pub(super) async fn handle_slash_setdialog(
    state: &mut GameState,
    conn_id: ConnectionId,
    args: &str,
) {
    ...
    if let Some(Some(npc)) = state.npcs.get_mut(npc_idx) {
        ...
        state.send_console(
            conn_id,
            &format!("Dialogo del NPC {} actualizado.", npc_idx),
            font_index::INFO,
        );
    } else {
...
pub(super) async fn handle_slash_reiniciar(
    state: &mut GameState,
    conn_id: ConnectionId,
    args: &str,
) {
    ...
    let admin_name = state
        ...
        .unwrap_or_default();
    let msg = format!(
        "ATENCION: El servidor se reiniciara en {} segundos.",
        seconds
    );
    state.send_console_to(SendTarget::ToAll, &msg, font_index::WARNING);
...
pub(super) async fn handle_slash_apass(state: &mut GameState, conn_id: ConnectionId, args: &str) {
    ...
    if parts.len() < 2 {
        state.send_console(
            conn_id,
            "Uso: /APASS <nombre>@<nuevapassword>",
            font_index::INFO,
        );
        return;
    }
    ...
    if new_pass.len() < 3 {
        state.send_console(
            conn_id,
            "La password debe tener al menos 3 caracteres.",
            font_index::INFO,
        );
        return;
    }
    ...
    let account_name = match state.find_user_by_name(target_name) {
        ...
        None => {
            state.send_console(
                conn_id,
                "Jugador no encontrado online. Solo funciona con jugadores conectados.",
                font_index::INFO,
            );
            return;
        }
        ...
        .is_ok()
    {
        ...
        state.send_console(
            conn_id,
            &format!("Password de {} cambiada.", target_name),
            font_index::INFO,
        );
...
pub(super) async fn handle_slash_banclan(
    state: &mut GameState,
    conn_id: ConnectionId,
    guild_name: &str,
) {
    ...
    for mc in &member_conns {
        ...
        state.send_console(
            *mc,
            "Tu clan ha sido baneado por un administrador.",
            font_index::FIGHT,
        );
    }
    ...
    state.send_console(
        conn_id,
        &format!(
            "Clan '{}' baneado. {} miembros online afectados.",
            guild_name,
            member_conns.len()
        ),
        font_index::INFO,
    ...
    info!(
        "[GM] {} banned guild {} ({} members affected)",
        admin_name,
        guild_name,
        member_conns.len()
    );
...
pub(super) async fn handle_slash_miembrosclan(
    state: &mut GameState,
    conn_id: ConnectionId,
    guild_name: &str,
) {
    ...
    if members.is_empty() {
        state.send_console(
            conn_id,
            &format!("No hay miembros de '{}' online.", guild_name),
            font_index::INFO,
        );
    } else {
        state.send_console(
...
pub(super) async fn handle_slash_rajarclan(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    if guild_name.is_empty() {
        state.send_console(
            conn_id,
            "El jugador no pertenece a ningun clan.",
            font_index::INFO,
        );
        return;
    }
    ...
    state.send_console(
        conn_id,
        &format!("{} ha sido removido del clan {}.", target_name, guild_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Has sido removido de tu clan por un administrador.",
        font_index::FIGHT,
    );
}
...
pub(super) async fn handle_slash_gm_request(
    state: &mut GameState,
    conn_id: ConnectionId,
    text: &str,
) {
    ...
    let request_pending = state
        .users
        .get(&conn_id)
        .map(|u| u.gm_request_pending)
        .unwrap_or(false);
    if request_pending {
        state.send_console(
            conn_id,
    ...
    let gm_conns: Vec<ConnectionId> = state
        .users
        .values()
    ...
    state.send_console(
        conn_id,
        "Tu solicitud ha sido enviada a los GMs online.",
        font_index::INFO,
    );
}
...
pub(super) async fn handle_slash_rol(state: &mut GameState, conn_id: ConnectionId, text: &str) {
    ...
    state.send_console(
        conn_id,
        "Tu solicitud de rol ha sido enviada.",
        font_index::INFO,
    );
}
...
pub(super) async fn handle_slash_elecciones(state: &mut GameState, conn_id: ConnectionId) {
    ...
    if !is_leader {
        state.send_console(
            conn_id,
            "Solo el lider puede abrir elecciones.",
            font_index::INFO,
        );
        return;
    }
    ...
    if state.guild_elections.contains_key(&guild_idx) {
        state.send_console(
            conn_id,
            "Ya hay elecciones abiertas en tu clan.",
            font_index::INFO,
        );
        return;
    }
    ...
    // Initialize election
    state.guild_elections.insert(
        guild_idx,
        crate::game::types::GuildElection {
            candidates: members,
    ...
    let msg = format!(
        "Se han abierto elecciones en el clan {}. Usa /VOTO <nombre> para votar.",
        guild_name
    );
    let member_conns: Vec<ConnectionId> = state
        .users
...
pub(super) async fn handle_centinela_improved(
    ...
    if typed == expected {
        ...
        state.send_console(
            conn_id,
            "Centinela verificado correctamente.",
        ...
        if fails >= 3 {
            ...
            state.send_console(
                conn_id,
                "Has fallado la verificacion centinela. Desconectado.",
                font_index::FIGHT,
            );
...
pub(super) async fn alertar_faccionarios(state: &mut GameState, conn_id: ConnectionId) {
    ...
    let faction_name = if armada {
        "Armada Real"
    } else {
        "Legion del Caos"
    };
    let msg = format!(
        "[{}] {} ha hecho sonar el cuerno de alerta! (Mapa {})",
        faction_name, name, map
    );
...
pub(super) async fn handle_slash_panelgm(state: &mut GameState, conn_id: ConnectionId) {
    ...
    if !state.sos_messages.is_empty() {
        state.send_console(
            conn_id,
            &format!("Solicitudes pendientes: {}", state.sos_messages.len()),
            font_index::SERVER,
        );
...
pub(super) async fn handle_slash_conden(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    if in_combat_zone {
        state.send_console(
            conn_id,
            "El jugador se encuentra en zona de pelea.",
            font_index::INFO,
        );
        return;
    }
    ...
    if was_armada {
        ...
        state.send_console(
            target_conn,
            "Has sido expulsado del ejercito real.",
            font_index::FIGHT,
        );
    }
    ...
    if let Some(u) = state.users.get_mut(&target_conn) {
        let l = (-u.rep_asesino - u.rep_bandido + u.rep_burgues - u.rep_ladrones
            + u.rep_noble
            + u.rep_plebe)
            / 6;
        u.criminal = l < 0;
    ...
    state.send_console(
        conn_id,
        &format!("{} ha sido condenado.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Un administrador te ha convertido en criminal.",
        font_index::FIGHT,
    );
...
pub(super) async fn handle_slash_rajar(state: &mut GameState, conn_id: ConnectionId, target: &str) {
    ...
    let target_conn = match state.find_user_by_name(target) {
        ...
        None => {
            state.send_console(
                conn_id,
                "Jugador no encontrado (debe estar online).",
                font_index::INFO,
            );
            return;
        }
    ...
    state.send_console(
        conn_id,
        &format!("Facciones de {} reseteadas.", target_name),
        font_index::INFO,
    );
    state.send_console(
        target_conn,
        "Tus datos de faccion han sido reseteados por un administrador.",
        font_index::FIGHT,
    );
...
pub(super) async fn handle_slash_forcemidimap(
    state: &mut GameState,
    conn_id: ConnectionId,
    args: &str,
) {
    ...
    state.send_console(
        conn_id,
        &format!(
            "MIDI {} enviado a {} jugadores en mapa {}.",
            midi_id, count, target_map
        ),
        font_index::INFO,
    );
    info!(
        "[GM] {} forced MIDI {} on map {} ({} players)",
        admin_name, midi_id, target_map, count
    );
}
...
pub(super) async fn handle_slash_aname(state: &mut GameState, conn_id: ConnectionId, args: &str) {
    ...
    if parts.len() < 2 {
        state.send_console(
            conn_id,
            "Uso: /ANAME <nombre_actual> <nombre_nuevo>",
            font_index::INFO,
        );
        return;
    }
    ...
    if new_name.is_empty() || new_name.len() > 20 {
        state.send_console(
            conn_id,
            "El nuevo nombre debe tener entre 1 y 20 caracteres.",
            font_index::INFO,
        );
        return;
    }
    ...
    if state.find_user_by_name(old_name).is_some() {
        state.send_console(
            conn_id,
            "El personaje debe estar offline para cambiar su nombre.",
            font_index::INFO,
        );
        return;
    }
    ...
    match crate::db::charfile::rename_character(&pool, &old_name_owned, &new_name_owned).await {
        Ok(true) => {
            ...
            state.send_console(
                conn_id,
                &format!(
                    "Personaje '{}' renombrado a '{}'.",
                    old_name_owned, new_name_owned
                ),
                font_index::INFO,
            );
            info!(
                "[GM] {} renamed character '{}' -> '{}'",
                admin_name, old_name_owned, new_name_owned
            );
        }
        Ok(false) => {
            state.send_console(
                conn_id,
                "Personaje no encontrado en la base de datos.",
                font_index::INFO,
            );
        }
        Err(_) => {
            state.send_console(
                conn_id,
                "Error al renombrar el personaje.",
                font_index::INFO,
            );
        }
    }
