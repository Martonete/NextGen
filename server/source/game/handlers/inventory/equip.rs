        ...
        if let Some(user) = state.users.get_mut(&conn_id) {
            ...
            match obj_data.obj_type {
                ...
                ObjType::Shield => {
                    ...
                    tracing::info!("[EQUIP] Shield slot={} obj={} '{}' shield_anim={}", slot, obj_index, obj_data.name, obj_data.shield_anim);
...
pub(crate) fn unequip_slot(state: &mut GameState, conn_id: ConnectionId, idx: usize, obj_type: &ObjType) {
