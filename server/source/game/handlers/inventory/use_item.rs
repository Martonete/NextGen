...
use crate::game::handlers::{naked_body, revive_user, send_inventory_slot, user_die};
...
pub(crate) async fn handle_use_item_click(
    state: &mut GameState,
    ...
    if is_projectile {
        ...
        if !is_equipped {
            state.send_console(
                conn_id,
                "Antes de usar la herramienta deberías equipártela.",
                font_index::INFO,
            );
            return;
        }
...
pub(crate) async fn handle_use_item_inner(
    state: &mut GameState,
    conn_id: ConnectionId,
    slot: usize,
    from_click: bool,
    ...
    match obj_data.obj_type {
        ObjType::UseOnce | ObjType::Potion => {
            ...
            if obj_data.tipo_pocion == 6 {
                ...
                if !is_gm {
                    state.send_console(
                        conn_id,
                        "Sientes un gran mareo y pierdes el conocimiento.",
                        font_index::FIGHT,
                    );
                    // Consume item first
            ...
            let (snd_map, snd_x, snd_y) = state
                .users
                .get(&conn_id)
                .map(|u| (u.pos_map, u.pos_x, u.pos_y))
                .unwrap_or((0, 0, 0));
            let pkt_wave =
                binary_packets::write_play_wave(snd_id as u8, snd_x as i16, snd_y as i16);
        ...
        ObjType::Key => {
            ...
            state.send_console(
                conn_id,
                "Esta llave sirve para abrir una puerta",
                font_index::INFO,
            );
        }
        ObjType::Boat => {
            ...
            if !is_navigating {
                ...
                // Pirata / Trabajador with Pesca >= 100: level 20. All others: level 25.
                {
                    ...
                    let min_level = if (user_class == PlayerClass::Pirata
                        || user_class == PlayerClass::Trabajador)
                        && fishing_skill >= 100
                    {
                        20
                    } else {
                    ...
                    if user_level < min_level {
                        state.send_console(
                            conn_id,
                            &format!(
                                "Necesitas nivel {} para abordar una embarcación.",
                                min_level
                            ),
                            font_index::INFO,
                        );
                ...
                if obj_data.min_skill > 0 {
                    ...
                    let modifier =
                        crate::game::handlers::skills::mod_navegacion(user_class, fishing_skill);
            ...
            if is_mounted_for_boat && !is_navigating {
                ...
                if let Some(u) = state.users.get_mut(&conn_id) {
                    ...
                    for slot_idx in 0..MAX_INVENTORY_SLOTS {
                        if !u.inventory[slot_idx].equipped {
                            continue;
                        }
                ...
                let (pkt_cd, pkt_au) = match state.users.get(&conn_id) {
                    Some(u) => (
                        ...
                        crate::game::handlers::common::build_aura_binary(u),
                    ),
                    None => return,
                };
                state.send_data_bytes(
            ...
            let (was_hidden, is_invisible, char_index_val, map_for_nover) =
            ...
            if was_hidden {
                ...
                if !is_invisible {
                    let pkt_invis =
                        binary_packets::write_set_invisible(char_index_val as i16, false, 0);
            ...
            if is_navigating {
                ...
                let equip_info = state.users.get(&conn_id).map(|u| {
                    let get_inv_obj = |slot: usize| -> i32 {
                        if slot >= 1 && slot <= u.inventory.len() {
                ...
                )) = equip_info
                {
                    let armor_body = state
                        .get_object(armor_obj)
                        .map(|o| o.num_ropaje)
                        .unwrap_or(0);
                    let weapon_anim = state
                        .get_object(weapon_obj)
                        .map(|o| o.weapon_anim)
                        .unwrap_or(0);
                    let shield_anim = state
                        .get_object(shield_obj)
                        .map(|o| o.shield_anim)
                        .unwrap_or(0);
                    let casco_anim = state
                        .get_object(helmet_obj)
                        .map(|o| o.casco_anim)
                        .unwrap_or(0);
                    if let Some(u) = state.users.get_mut(&conn_id) {
                        u.navigating = false;
                        ...
                        if !dead {
                            u.head = saved_head;
                            u.body = if armor_body > 0 {
                                armor_body
                            } else {
                                naked_body(race, gender)
                            };
        ...
        ObjType::Scroll => {
            ...
            if let Some(slot_idx) = slot {
                ...
                let pkt_shs = binary_packets::write_change_spell_slot(
                    shs_slot as u8,
                    spell_id as i16,
                    &spell_name,
                );
        ...
        ObjType::Mount => {
            ...
            // Pre-checks
            let (is_dead, is_navigating, is_mounted) = state
                .users
                .get(&conn_id)
            ...
            let (pkt_cd, pkt_au) = {
                ...
                (
                    crate::game::handlers::common::build_cd_binary(user),
                    crate::game::handlers::common::build_aura_binary(user),
                )
            };
...
pub(crate) fn apply_consumable(
    state: &mut GameState,
    conn_id: ConnectionId,
    obj: &crate::data::objects::ObjData,
) {
    if let Some(user) = state.users.get_mut(&conn_id) {
        ...
        match obj.tipo_pocion {
            1 => {
                ...
                let new_agi = (user.attributes[1] + amount)
                    .min(40)
                    .min(2 * user.attributes_backup[1]);
            ...
            2 => {
                ...
                let new_str = (user.attributes[0] + amount)
                    .min(40)
                    .min(2 * user.attributes_backup[0]);
