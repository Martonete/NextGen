...
pub(crate) async fn accion_para_puerta(
    state: &mut GameState,
        ...
        let has_key = state
            ...
            .map(|u| {
                u.inventory.iter().any(|s| {
                    ...
                    state
                        .get_object(s.obj_index)
                        .map(|ko| {
                            ko.obj_type == crate::data::objects::ObjType::Key
                                && ko.clave == obj.clave
                        })
                        .unwrap_or(false)
                })
    ...
    if obj.cerrada == 1 {
        ...
        let is_puerta_doble = new_obj
            .as_ref()
            .map(|o| o.puerta_doble == 1)
            .unwrap_or(false);
        ...
        let is_puerta_doble = closed_obj
            .as_ref()
            .map(|o| o.puerta_doble == 1)
            .unwrap_or(false);
...
pub(crate) async fn accion_para_foro(state: &mut GameState, conn_id: ConnectionId, obj_index: i32) {
    ...
    send_forum_posts(
        ...
        conn_id,
        &foro_id,
        forum_msg_type::GENERAL,
        forum_msg_type::GENERAL_STICKY,
    )
    .await;
    ...
    if is_caos || is_gm {
        visibility |= forum_visibility::CAOS_MEMBER;
    }
    if is_armada || is_gm {
        visibility |= forum_visibility::REAL_MEMBER;
    }
...
async fn send_forum_posts(
    ...
    for post in &forum.posts {
        let pkt = binary_packets::write_add_forum_msg(
            regular_type,
            &post.title,
            &post.author,
            &post.body,
    ...
    for post in &forum.stickies {
        let pkt =
            binary_packets::write_add_forum_msg(sticky_type, &post.title, &post.author, &post.body);
        state.send_bytes(conn_id, &pkt);
    }
}
...
pub(crate) async fn handle_forum_post(
    state: &mut GameState,
    conn_id: ConnectionId,
    msg_type: u8,
    title: String,
    ...
    if target_obj <= 0 || title.is_empty() || body.is_empty() {
        return;
    }
