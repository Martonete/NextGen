...
use super::{
    check_user_level, naked_body, send_stats_exp, send_stats_gold, send_stats_hp, send_stats_mana,
    send_stats_sta, send_warp_fx, warp_user,
};
        ...
        Some((name, map, x, y, target_priv)) => {
            ...
            if target_priv >= privilege_level::DIOS {
                ...
                if my_priv < target_priv {
                    state.send_console(
                        conn_id,
                        "No podes localizar a ese usuario.",
                        font_index::INFO,
                    );
                    return;
                }
...
pub(super) async fn handle_slash_revivir(
    state: &mut GameState,
    ...
    match target_conn {
        Some(tc) => {
            ...
            let (heading, weapon_anim, shield_anim, casco_anim, char_index) =
                match state.users.get(&tc) {
                    Some(u) => (
                        u.heading,
                        u.weapon_anim,
                        u.shield_anim,
...
pub(super) async fn handle_slash_espiar(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
...
pub(super) async fn handle_slash_inv(state: &mut GameState, conn_id: ConnectionId, target: &str) {
    ...
    match found {
        Some(tc) => {
            ...
            let inv_data: Vec<(usize, i32, i32, bool)> = state
                .users
                .get(&tc)
                .map(|u| {
                    u.inventory
            ...
            state.send_console(
                conn_id,
                &format!("Inventario de {}:", target_name),
                font_index::INFO,
            );
            ...
            for (slot, obj_idx, amount, equipped) in inv_data {
                ...
                state.send_console(
                    conn_id,
                    &format!("  Slot {}: {} x{}{}", slot, name, amount, eq),
                    font_index::INFO,
                );
            }
...
pub(super) async fn handle_slash_bov(state: &mut GameState, conn_id: ConnectionId, target: &str) {
    ...
    match found {
        Some(tc) => {
            let bank_data: Vec<(usize, i32, i32)> = state
                .users
                .get(&tc)
                .map(|u| {
                    u.bank
            ...
            state.send_console(
                conn_id,
                &format!("Boveda de {} (Oro: {}):", target_name, bank_gold),
                font_index::INFO,
            );
            ...
            for (slot, obj_idx, amount) in bank_data {
                ...
                state.send_console(
                    conn_id,
                    &format!("  Slot {}: {} x{}", slot, name, amount),
                    font_index::INFO,
                );
            }
...
pub(super) async fn handle_slash_nick2ip(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
...
pub(super) async fn handle_slash_mod(
    state: &mut GameState,
    conn_id: ConnectionId,
    args: &str,
    _is_self: bool,
...
pub(super) async fn apply_mod_other(
    state: &mut GameState,
    gm_conn: ConnectionId,
    target: ConnectionId,
    stat: &str,
    value: i64,
) {
    ...
    match stat {
        ...
        "ORO" => {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@oro@{}@{}", gm_name, target_name, value),
            );
        }
        "EXP" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@experiencia@{}@{}", gm_name, target_name, value),
            );
        }
        "BODY" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@body@{}@{}", gm_name, target_name, value),
            );
        }
        "HEAD" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@head@{}@{}", gm_name, target_name, value),
            );
        }
        "CRI" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@criminales@{}@{}", gm_name, target_name, value),
            );
        }
        "CIU" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@ciudadanos@{}@{}", gm_name, target_name, value),
            );
        }
        "LEVEL" => {
            // Hard cap at MAX_LEVEL (50) so no edit can push past the level cap.
        ...
        "STA" => {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@energia@{}@{}", gm_name, target_name, value),
            );
        }
        "MP" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@mana@{}@{}", gm_name, target_name, value),
            );
        }
        "HP" => {
            if let Some(user) = state.users.get_mut(&target) {
            ...
            state.send_msg_id_to(
                SendTarget::ToAdmins,
                591,
                &format!("{}@vida@{}@{}", gm_name, target_name, value),
            );
        }
        _ => {
...
pub(super) async fn handle_slash_dv(state: &mut GameState, conn_id: ConnectionId, target: &str) {
    ...
    state.send_console(
        target_id,
        "Un GM te ha devuelto a tu posicion anterior.",
        font_index::INFO,
    );
    state.send_console(
        conn_id,
        &format!("Jugador '{}' devuelto.", target),
        font_index::INFO,
    );
}
...
pub(super) async fn handle_slash_info(state: &mut GameState, conn_id: ConnectionId, target: &str) {
    ...
    let target_id = match state.find_user_by_name(target) {
        ...
        None => {
            state.send_console(
                conn_id,
                &format!("Jugador '{}' no esta online.", target),
                font_index::INFO,
            );
            return;
        }
    ...
    let info_lines = match state.users.get(&target_id) {
        Some(u) => {
            vec![
                ...
                format!(
                    "HP: {}/{} | Mana: {}/{} | Sta: {}/{}",
                    u.min_hp, u.max_hp, u.min_mana, u.max_mana, u.min_sta, u.max_sta
                ),
                format!(
                ...
                format!(
                    "Mapa: {} ({},{}) | IP: {}",
                    u.pos_map, u.pos_x, u.pos_y, u.ip
                ),
                format!(
                    "Criminal: {} | Muerto: {} | Privilegios: {}",
                    u.criminal, u.dead, u.privileges
                ),
...
pub(super) async fn handle_slash_edit(state: &mut GameState, conn_id: ConnectionId, args: &str) {
    ...
    state.send_console(
        conn_id,
        &format!("Jugador '{}' ahora es nivel {}.", target_name, final_level),
        font_index::INFO,
    );
...
pub(super) async fn handle_slash_lastip(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
...
pub(super) async fn handle_slash_consulta(
    state: &mut GameState,
    conn_id: ConnectionId,
    target: &str,
) {
    ...
    state.send_console(
        conn_id,
        &format!("{} ahora esta {}.", target_name, status),
        font_index::INFO,
    );
...
pub(super) async fn handle_slash_onlinereal(state: &mut GameState, conn_id: ConnectionId) {
    ...
    if members.is_empty() {
        state.send_console(
            conn_id,
            "No hay miembros de la Armada Real online.",
            font_index::INFO,
        );
    } else {
        state.send_console(
            conn_id,
            &format!("Armada Real online ({}):", members.len()),
            font_index::INFO,
        );
...
pub(super) async fn handle_slash_onlinecaos(state: &mut GameState, conn_id: ConnectionId) {
    ...
    if members.is_empty() {
        state.send_console(
            conn_id,
            "No hay miembros de la Legion del Caos online.",
            font_index::INFO,
        );
    } else {
        state.send_console(
            conn_id,
            &format!("Legion del Caos online ({}):", members.len()),
            font_index::INFO,
        );
        state.send_console(conn_id, &members.join(", "), font_index::INFO);
    }
