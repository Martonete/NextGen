...
pub(super) async fn send_lookat_user_info(
    state: &mut GameState,
    ...
    )) = info
    {
        ...
        if armada {
            stat.push_str(" <Alianza Imperial>");
        } else if caos {
            stat.push_str(" <Horda Infernal>");
        }
        if desc.len() > 1 {
...
pub(super) async fn send_lookat_npc_info(
    state: &mut GameState,
    conn_id: ConnectionId,
    npc_idx: usize,
) {
    ...
    if let Some((npc_name, npc_desc, _npc_ci, min_hp, max_hp)) = npc_data {
        let is_gm = state
            .users
            .get(&conn_id)
            .map(|u| u.privileges >= crate::game::types::privilege_level::SEMIDIOS)
            .unwrap_or(false);
...
pub(super) async fn apply_spell_invocation(
    ...
    if is_invocar_blocked_at(state, map, x, y) {
        state.send_console(
            caster_id,
            "No puedes invocar criaturas aqui.",
            font_index::INFO,
        );
        return;
    }
    ...
    if nro_mascotas >= MAX_MASCOTAS {
        state.send_console(
            caster_id,
            "No puedes invocar mas criaturas.",
            font_index::INFO,
        );
        return;
    }
    ...
    // VB6: Elemental singleton — if already summoned, warp it to caster + reset lifetime
    {
        ...
        if already_has {
            ...
            let existing_idx = state.users.get(&caster_id).and_then(|u| {
                for slot in 0..3 {
            ...
            if let Some(npc_idx) = existing_idx {
                ...
                let old_data = state
                    .get_npc(npc_idx)
                    .map(|n| (n.char_index, n.map, n.x, n.y));
...
pub(super) async fn apply_mimetiza_user(
    state: &mut GameState,
    caster_id: ConnectionId,
    target_id: ConnectionId,
) {
...
pub(super) async fn apply_mimetiza_npc(
    state: &mut GameState,
    caster_id: ConnectionId,
    npc_idx: usize,
) {
